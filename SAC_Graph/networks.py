...
class QNetwork(nn.Module):
    ...
    def forward(self, adj, edge, state, action):
        ...
        e_agg_u = torch.sum(e * F.softmax(self.e_att(e), dim=1), dim=1)  # [B, 1, eh_size]
        v_agg_u = torch.sum(v * F.softmax(self.v_att(v), dim=1), dim=1)  # [B, 1, vh_size]
...
class GauusianPolicy(nn.Module):
    def __init__(self, config: GraphSacConfig):
        super().__init__()
        self.state_size = get_size(config.state_shape)
        self.edge_size = get_size(config.edge_shape)
...
class GCNBlock(nn.Module):
    ...
    def forward(self, B, N_max, adj, adj_T, e_feature, v_feature, u_feature):
        ...
        """
        v_s = torch.matmul(adj, torch.transpose(e_feature, 2, 1))
        ...
        e_agg_v = torch.matmul(adj_T, torch.transpose(e_out, 2, 1))
        v_in = torch.cat([
            e_agg_v,
            e_out,
            v_feature,
        ...
        e_agg_u = torch.sum(e_out * F.softmax(self.e_att(e_out), dim=1), dim=1)  # [B, 1, eh_size]
        v_agg_u = torch.sum(v_out * F.softmax(self.v_att(v_out), dim=1), dim=1)  # [B, 1, vh_size]
